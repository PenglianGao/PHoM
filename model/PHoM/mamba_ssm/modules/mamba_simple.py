...
class HOM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, extra_emb=None, **kwargs):
        if self.bimamba_type == "v3":
            ...
            xp11 = self.proj1(xp1 * F.silu(z) + xp1)
            xp12 = self.proj2(xp2 * F.silu(x11) + xp2)
            xp13 = self.out_proj(xp3 * F.silu(x12) + xp3)
            out_p = xp13
...
class HOM_0_order(nn.Module):
    def __init__(
        ...
        # 层定义
        #self.in_proj = nn.Linear(self.d_model, self.d_model * 2, bias=bias, **factory_kwargs)
        self.in_proj = nn.Linear(self.d_model, self.d_model * 1, bias=bias, **factory_kwargs)
        self.act = nn.SiLU()
    ...
    def forward(self, x: torch.Tensor, extra_emb=None, **kwargs):
        if self.bimamba_type == "v3":
            ...
            #x11 = self.proj1(x1 * F.silu(z) + x1)
            #x12 = self.proj2(x2 * F.silu(x11) + x2)
            x13 = self.out_proj(x1)
            out = x13

            # xp11 = self.proj1(xp1 * F.silu(z) + xp1)
            # xp12 = self.proj2(xp2 * F.silu(x11) + xp2)
            xp13 = self.out_proj(xp1)
            out_p = xp13
...
class HOM_1_order(nn.Module):
    def __init__(
        ...
        #self.in_proj = nn.Linear(self.d_model, self.d_model * 2, bias=bias, **factory_kwargs)
        #self.in_proj = nn.Linear(self.d_model, self.d_model * 1, bias=bias, **factory_kwargs)
        self.in_proj = nn.Linear(self.d_model, self.d_model * int(1+1), bias=bias, **factory_kwargs)
        self.act = nn.SiLU()
    ...
    def forward(self, x: torch.Tensor, extra_emb=None, **kwargs):
        if self.bimamba_type == "v3":
            ...
            # 拆分并处理
            #z, x1, x2, x3 = torch.split(x, [C // 4, C // 4, C // 2, C], dim=3)
            z, x3 = torch.split(x, [C , C], dim=3)
            #zp, xp1, xp2, xp3 = torch.split(xp, [C // 4, C // 4, C // 2, C], dim=3)
            zp, xp3 = torch.split(xp, [C , C], dim=3)
            ...
            # x11 = self.proj1(x1 * F.silu(z) + x1)
            #x12 = self.proj2(x2 * F.silu(x11) + x2)
            x13 = self.out_proj(x3 * F.silu(z) + x3)
            out = x13

            # xp11 = self.proj1(xp1 * F.silu(z) + xp1)
            # xp12 = self.proj2(xp2 * F.silu(x11) + xp2)
            xp13 = self.out_proj(xp3 * F.silu(zp) + xp3)
            out_p = xp13
...
class HOM_2_order(nn.Module):
    ...
    def forward(self, x: torch.Tensor, extra_emb=None, **kwargs):
        if self.bimamba_type == "v3":
            ...
            # 拆分并处理
            #z, x1, x2, x3 = torch.split(x, [C // 4, C // 4, C // 2, C], dim=3)
            z, x2, x3 = torch.split(x, [C // 2, C // 2, C], dim=3)
            #zp, xp1, xp2, xp3 = torch.split(xp, [C // 4, C // 4, C // 2, C], dim=3)
            zp, xp2, xp3 = torch.split(xp, [C // 2, C // 2, C], dim=3)
            ...
            # 输出投影
            # x11 = self.proj1(x1 * F.silu(z) + x1)
            x12 = self.proj2(x2 * F.silu(z) + x2)
            x13 = self.out_proj(x3 * F.silu(x12) + x3)
            out = x13

            # xp11 = self.proj1(xp1 * F.silu(z) + xp1)
            xp12 = self.proj2(xp2 * F.silu(zp) + xp2)
...
class HOM_4_order(nn.Module):
    def __init__(
        ...
        self.SSD0 = SS2D(d_model=d_model // 8, d_state=d_state // 8, expand=expand, dropout=dropout, **kwargs)##4-order
        self.SSD1 = SS2D(d_model=d_model // 4, d_state=d_state // 4, expand=expand, dropout=dropout, **kwargs)
        self.SSD2 = SS2D(d_model=d_model // 2, d_state=d_state // 2, expand=expand, dropout=dropout, **kwargs)
        self.SSD3 = SS2D(d_model=d_model, d_state=d_state, expand=expand, dropout=dropout, **kwargs)
        
        
        self.SSDp0 = SS2D(d_model=d_model // 8, d_state=d_state // 8, expand=expand, dropout=dropout, **kwargs)##4-order
        self.SSDp1 = SS2D(d_model=d_model // 4, d_state=d_state // 4, expand=expand, dropout=dropout, **kwargs)
        self.SSDp2 = SS2D(d_model=d_model // 2, d_state=d_state // 2, expand=expand, dropout=dropout, **kwargs)
        self.SSDp3 = SS2D(d_model=d_model, d_state=d_state, expand=expand, dropout=dropout, **kwargs)
        ...
        # 输出层
        self.out_norm = nn.LayerNorm(self.d_inner)
        self.proj0 = nn.Linear(self.d_inner // 8, self.d_model // 4, bias=bias, **factory_kwargs)##4-order
        self.proj1 = nn.Linear(self.d_inner // 4, self.d_model // 2, bias=bias, **factory_kwargs)
        self.proj2 = nn.Linear(self.d_inner // 2, self.d_model, bias=bias, **factory_kwargs)
        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)
    ...
    def forward(self, x: torch.Tensor, extra_emb=None, **kwargs):
        if self.bimamba_type == "v3":
            ...
            # z, x1, x2, x3 = torch.split(x, [C // 4, C // 4, C // 2, C], dim=3)
            # zp, xp1, xp2, xp3 = torch.split(xp, [C // 4, C // 4, C // 2, C], dim=3)
            z, x0, x1, x2, x3 = torch.split(x, [C // 8, C // 8, C // 4, C // 2, C], dim=3)
            zp, xp0, xp1, xp2, xp3 = torch.split(xp, [C // 8, C // 8, C // 4, C // 2, C], dim=3)
            ...
            # 输出投影
            x10 = self.proj0(x0 * F.silu(z) + x0)
            x11 = self.proj1(x1 * F.silu(x10) + x1)
            x12 = self.proj2(x2 * F.silu(x11) + x2)
            x13 = self.out_proj(x3 * F.silu(x12) + x3)
            out = x13

            xp10 = self.proj0(xp0 * F.silu(z) + xp0)
            xp11 = self.proj1(xp1 * F.silu(xp10) + xp1)
